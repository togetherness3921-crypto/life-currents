import React, { useState, FormEvent, useEffect, useRef, FocusEvent } from 'react';
...
const ChatPane = () => {
    ...
    const submitMessage = async (content: string, threadId: string, parentId: string | null) => {
        ...
        let toolsForRequest: ApiToolDefinition[] | undefined =
        ...
        if (toolsForRequest && isToolIntentCheckEnabled(selectedModel.id)) {
            try {
                ...
                console.log('[ChatPane][Intent] Classified intent as', intent);
                if (intent === 'CONVERSATION') {
                    toolsForRequest = undefined;
                }
            } catch (intentError) {
                console.warn('[ChatPane][Intent] Failed to classify intent, defaulting to tools', intentError);
            }
        }
    ...
    const handleInputFocusWithin = () => {
        setIsInputFocused(true);
    };

    const handleInputBlurWithin = (event: FocusEvent<HTMLDivElement>) => {
    ...
    return (
        <div className="relative flex h-full flex-col bg-background">
            <ScrollArea className="flex-1 p-4 pb-[200px]" ref={scrollAreaRef}>
            ...
            <div
                className={cn(
                    'absolute inset-x-0 bottom-0 border-t bg-background transition-all duration-300 ease-in-out',
                    isInputFocused ? 'h-1/2 shadow-xl' : 'h-[140px]'
                )}
                onFocusCapture={handleInputFocusWithin}
                onBlurCapture={handleInputBlurWithin}
            >
                <form
                    onSubmit={handleSubmit}
                    className={cn(
                        'flex h-full gap-3 px-4 py-4 transition-all duration-300',
                        isInputFocused ? 'flex-col' : 'flex-row items-center'
                    )}
                >
                    <Textarea
                        value={input}
                        ...
                        disabled={isLoading}
                        className={cn(
                            'resize-none transition-all duration-300 !min-h-[48px]',
                            isInputFocused ? 'flex-1 !min-h-0' : 'h-12'
                        )}
                    />
                    <div
                        className={cn(
                            'flex shrink-0 flex-wrap items-center gap-2',
                            isInputFocused ? 'w-full justify-end' : 'justify-start'
                        )}
                    >
                        <Button
                            type="button"
